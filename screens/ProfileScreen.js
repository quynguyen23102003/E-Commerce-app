...
const ProfileScreen = () => {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        const response = await axios.get(`${IPCONFIG.CONSTANTS.IP}profile/${userId}`);
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get(`${IPCONFIG.CONSTANTS.IP}orders/${userId}`);
  ...
  return (
    <ScrollView style={{padding: 10, flex: 1, backgroundColor: 'white'}}>
      ...
      <View style={{flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 12}}>
        ...
        <Pressable style={{padding: 10, backgroundColor: '#E0E0E0', borderRadius: 25, flex: 1}} onPress={logout}>
